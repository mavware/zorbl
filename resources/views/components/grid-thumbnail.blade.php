...
<div
    {{ $attributes->merge(['class' => ($fluid ? 'grid w-full' : 'inline-grid').' gap-px p-px '.$frameClass]) }}
    style="grid-template-columns: repeat({{ $width }}, minmax(0, 1fr));{{ $fluid ? '' : ' width: '.min($width * $cellSize, $maxWidth).'px;' }}"
>
