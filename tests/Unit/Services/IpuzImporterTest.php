...
beforeEach(function () {
    $this->importer = app(IpuzImporter::class);
});
...
it('handles dict-format clues', function () {
    $ipuz = json_encode([
        'version' => 'http://ipuz.org/v2',
        'kind' => ['http://ipuz.org/crossword#1'],
        ...
        'clues' => [
            'Across' => [
                ['number' => 1, 'clue' => 'A dict clue', 'answer' => 'ABC'],
            ],
            'Down' => [],
        ],
    ]);
    ...
    expect($result['clues_across'][0]['clue'])->toBe('A dict clue');
});
