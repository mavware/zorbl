...
class PuzExporter
{
    ...
    /**
     * Validate that the crossword can be exported to .puz without data loss.
     *
    ...
    public function validate(Crossword $crossword): void
    {
        ...
        if ($crossword->hasVoidCells()) {
            $issues[] = UnsupportedFeature::VoidCells;
        }
        ...
        if ($crossword->hasNonLatin1Content()) {
            $issues[] = UnsupportedFeature::NonAscii;
        }

        if ($issues !== []) {
            throw new ExportValidationException('PUZ', $issues);
        }
    }

    /**
     * Export a Crossword to .puz binary format.
     *
     * @throws ExportValidationException
     */
    ...
    private function orderCluesForPuz(Crossword $crossword): array
    {
        ...
        $acrossMap = [];
        foreach ($crossword->clues_across as $clue) {
            $acrossMap[$clue['number']] = $clue;
        }

        $downMap = [];
        foreach ($crossword->clues_down as $clue) {
            $downMap[$clue['number']] = $clue;
        }
        ...
        foreach ($allNumbers as $number) {
            if (isset($acrossNumbers[$number])) {
                $ordered[] = $acrossMap[$number]['clue'] ?? '';
            }
            if (isset($downNumbers[$number])) {
                $ordered[] = $downMap[$number]['clue'] ?? '';
            }
        }
    ...
    private function buildGextSection(Crossword $crossword): string
    {
        ...
        foreach ($styles as $key => $style) {
            if (isset($style['shapebg']) && $style['shapebg'] === 'circle') {
                ...
                if ($index < $boardSize) {
                    $data[$index] = chr(self::GEXT_CIRCLE_FLAG);
                    $hasCircles = true;
                }
            }
        }
